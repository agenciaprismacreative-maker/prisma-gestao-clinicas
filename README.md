...
Vendas com carrinho de vários serviços na mesma venda (ou preenchido de uma vez a partir de um modelo de pacote cadastrado no Financeiro) e produtos de revenda com aviso de nível de estoque (amarelo perto do mínimo, vermelho no mínimo ou zerado) direto na lista de seleção — produto sem estoque não pode ser adicionado, nem em quantidade maior do que a disponível —, quantidade de sessões e desconto (ou cortesia) por item, desconto percentual sobre o total do plano, até duas formas de pagamento combinadas na mesma venda com parcelamento e maquininha, origem da venda estruturada (indicação de paciente ou de funcionário, sem precisar informar percentual: ao aprovar a venda, a comissão de quem indicou é calculada sozinha pela faixa de comissão de meta individual que essa pessoa estiver atingindo naquele momento em Desempenho › Metas), seleção de qual profissional é o responsável pela venda (é esse profissional que recebe o crédito nas metas, no ranking e na comissão; só o administrador pode escolher outro nome, Atendente e Esteticista têm a venda sempre fixada no próprio nome), um popup de detalhes da venda com o vendedor responsável, quem indicou, forma(s) de pagamento e parcelamento detalhados, e um comprovante em PDF com layout mais organizado (cabeçalho com a cor da marca, dados fiscais, vendedor) que abre em pré-visualização antes de baixar. A venda entra como pendente e só depois de aprovada é que gera os pacotes de sessão no cadastro do paciente, o lançamento no financeiro (atribuído ao profissional responsável escolhido na venda, para fins de meta e comissão) e, quando indicada por um funcionário, o lançamento automático da comissão. Cancelamento de uma venda pendente exige descrever o motivo. Esteticista também pode criar vendas (com atalho na tela de Atendimento), que seguem para aprovação do administrador como qualquer outra. O menu lateral mostra um pontinho amarelo no item Vendas, visível só para administrador, sempre que houver alguma venda pendente aguardando aprovação.
